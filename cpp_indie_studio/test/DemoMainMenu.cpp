bool CMainMenu::run()
{
  ...
  // NOTE: this guiven as IEventReceiver
  ...
  // NOTE: Create multiple checkboxes
  const s32 d = 50;
  ...
  // NOTE: Add a 3D model to the scene
...
bool CMainMenu::OnEvent(const SEvent& event)
{
  if (event.EventType == EET_KEY_INPUT_EVENT &&
    ...
    {
      // NOTE: When clicked on a GUI Element
      s32 id = event.GUIEvent.Caller->getID();
      switch(id)
	{
	case 45:
	  // NOTE: Click on Context menu
